...
public class Search {
   ...
   private void addMatches(String line, Pattern pattern) {
      ...
      while (matcher.find())
         matches.add(createMatch(line, matcher.start(), matcher.end()));
   }

   private Match createMatch(String line, int start, int end) {
      start -= surroundingCharacterCount;
      if (start <= 0) start = 0;
      end += surroundingCharacterCount;
      if (end >= line.length()) end = line.length();
      return new Match(searchTitle, searchString, line.substring(start, end));
   }

   private void search() throws IOException {
      var pattern = Pattern.compile(searchString);
      matches.clear();
